// src/pages/AdminProductTransfer.jsx – all buttons now use 10px 16px padding
...
export default function AdminProductTransfer() {
  ...
  const availableProducts = [
    ...
    {
      value: "Nail Polish Set",
      label: "Nail Polish Set",
      available: 75,
      price: 250,
    },
    {
  ...
  const handleAddProduct = () => {
    ...
    if (transferQty < 1 || transferQty > selectedProduct.available)
      return alert(
        `Quantity must be between 1 and ${selectedProduct.available}`,
      );
    if (products.some((p) => p.name === selectedProduct.value))
      return alert("This product is already in the transfer list!");
  ...
  return (
    <AdminLayout title="">
      <div className="min-h-screen bg-white pb-40 sm:pb-20">
        {/* White container shortened */}
                ...
                {filtered.map((p) => (
                  <tr key={p.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-medium ${p.status === "added" ? "bg-orange-100 text-orange-700" : "bg-gray-100 text-gray-700"}`}
                      >
                        {p.remarks}
                      </span>
                    </td>
        ...
        {/* SUBMIT BUTTONS – now match Add Product's new 10px 16px padding */}
        <div className="flex justify-end gap-4 pt-6 border-t border-gray-200">
          <button className="inline-flex items-center justify-center gap-2 px-[16px] py-[10px] border border-gray-300 rounded-full font-medium text-gray-700 hover:bg-gray-50 transition text-xs whitespace-nowrap min-w-[100px]">
            Cancel
          </button>
          <button
            className="inline-flex items-center justify-center gap-2 px-[16px] py-[10px] text-white font-bold text-xs whitespace-nowrap hover:scale-103 transition-all duration-200 hover:shadow-xl min-w-[120px] rounded-full"
            style={{
