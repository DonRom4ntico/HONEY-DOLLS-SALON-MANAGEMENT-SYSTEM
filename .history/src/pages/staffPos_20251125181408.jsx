...
export default function StaffPOS() {
  ...
  const [amountPaidInput, setAmountPaidInput] = useState(""); // input field
  const [amountPaid, setAmountPaid] = useState(0); // confirmed value
  ...
  // Trigger print when amountPaid changes
  useEffect(() => {
  ...
  const handleConfirmPay = async () => {
    ...
    try {
      ...
      // Set the confirmed amount paid (triggers print)
      ...
      // Only clear the input field, cart remains for printing
      setAmountPaidInput("");
    } catch (err) {
      console.error(err);
