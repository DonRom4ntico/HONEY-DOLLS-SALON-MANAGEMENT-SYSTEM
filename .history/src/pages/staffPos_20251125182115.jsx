...
export default function StaffPOS() {
  ...
  const [amountPaid, setAmountPaid] = useState(0); // confirmed value for receipt
  ...
  const handleConfirmPay = async () => {
    ...
    try {
      ...
      // Set the confirmed amount paid (updates receipt)
      ...
      // NOTE: Do NOT clear cart here, so receipt remains
      setAmountPaidInput("");
    } catch (err) {
      console.error(err);
  ...
  // ---------------- Ensure printing works after amountPaid updates ----------------
  useEffect(() => {
    if (amountPaid > 0 && receiptRef.current) {
      // Only print when user clicks print button
      // This effect can be removed if you don't want auto-print
    }
  }, [amountPaid]);
        ...
        <div className="grid lg:grid-cols-[2fr_1fr] gap-8 h-[calc(100vh-100px)]">
          ...
          <div className="bg-gray-50 rounded-2xl p-5 flex flex-col h-full overflow-y-auto">
            ...
            <div className="space-y-3 mt-4">
              ...
              // Replace your print button click with this:
              <button
                onClick={() => {
                  if (receiptRef.current) {
                    // Small timeout ensures React updates DOM
                    setTimeout(() => {
                      handlePrint();
                    }, 50);
                  } else {
                    alert("Receipt is not ready yet!");
                  }
                }}
