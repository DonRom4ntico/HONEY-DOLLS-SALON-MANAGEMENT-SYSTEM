...
export default function StaffPOS() {
  ...
  const [selectedCategory, setSelectedCategory] = useState("All"); // filter for products
  const [selectedServiceType, setSelectedServiceType] = useState("All"); // filter for services
  ...
  const filteredServices = services
    ...
    .filter(
      (s) =>
        selectedServiceType === "All" || s.servicetype === selectedServiceType
    );

  // Unique categories and service types
    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-50 to-pink-50 flex flex-col">
      ...
      <main className="flex-1 px-6 py-8 max-w-7xl mx-auto w-full">
        <div className="grid lg:grid-cols-4 gap-6">
          {/* Sidebar Filters */}
          <aside className="lg:col-span-1 bg-white p-4 rounded-2xl shadow">
            <h3 className="font-bold mb-3">Filters</h3>

            <div className="mb-4">
              <label className="font-medium mb-1 block">Product Category</label>
              <select
          ...
          <section className="lg:col-span-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[...filteredProducts, ...filteredServices].map((item) => {
                const added =
                  cart.find((i) => i.id === item.id && i.type === item.type)
                    ?.quantity || 0;
                return (
                  <div
          ...
          <div className="flex-1 space-y-2 mb-4 overflow-y-auto max-h-96">
            {cart.length === 0 ? (
              ...
              cart.map((item) => (
                <div
                  key={`${item.type}-${item.id}`}
                  className="bg-gray-50 rounded-xl p-3 flex items-center justify-between text-sm"
                >
