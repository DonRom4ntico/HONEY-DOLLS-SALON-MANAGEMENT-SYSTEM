...
export default function StaffLayout({ children }) {
  ...
  const staffName = "Anna"; // dynamic if needed
    ...
    <div className="min-h-screen flex flex-col text-gray-900 font-sans">
      ...
      <header className="bg-gradient-to-r from-[#ffd36e] to-[#f59e9e] shadow-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          {/* NAV LINKS (DESKTOP) */}
          <nav className="hidden lg:flex items-center gap-10 text-orange-900 font-semibold">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
        ...
        <div className="lg:hidden px-6 pb-4">
          ...
          {mobileMenuOpen && (
            <nav className="flex flex-col gap-4 text-orange-900 font-semibold mt-2">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.path}
