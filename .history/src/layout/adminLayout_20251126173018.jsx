...
export default function AdminLayout({ children, title }) {
  ...
  const menuItems = [
    ...
    {
      icon: Trash2,
      label: "Record Product Waste",
      path: "/recordProductWaste",
    },
    {
      icon: AlertTriangle,
      label: "Record Product Damaged",
      path: "/adminProductDamage",
    },
    {
      icon: DollarSign,
  ...
  const isReturnedProductsActive = [
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-50 to-pink-50 flex flex-col">
        ...
        <div className="px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-orange-900 font-medium hidden sm:inline">
              Welcome,{" "}
              <span className="text-yellow-600 font-bold">Alfa Smith</span>
            </span>
          </div>
        </div>
      </header>
      ...
      <div className="flex flex-1 relative">
        {/* DESKTOP SIDEBAR */}
        <aside
          className={` lg:flex flex-col bg-white shadow-lg transition-all duration-300 ${
            sidebarOpen ? "w-64" : "w-20"
          }`}
        >
          <div className="p-4 border-b flex items-center justify-between">
            {sidebarOpen && (
              <h2 className="text-sm font-bold text-orange-900">Admin</h2>
            )}
            <button
          ...
          <>
            ...
            <aside className="fixed top-0 left-0 bottom-0 w-72 bg-white shadow-2xl z-50 flex flex-col lg:hidden transition-transform duration-300 translate-x-0">
                ...
                {menuItems.map((item, idx) => {
                  if (item.header)
                    return (
                      <div
                        key={idx}
                        className="text-xs font-bold text-gray-500 uppercase mt-6 mb-3 px-3"
                      >
                        {item.header}
                      </div>
                    );
                          ...
                          <div className="ml-10 mt-2 space-y-1 bg-gray-50 rounded-xl p-3">
                            {item.items.map((sub, i) => (
                              <Link
                                ...
                                to={sub.path}
                                onClick={() => setMobileMenuOpen(false)}
                                className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-orange-100 transition font-medium text-gray-700"
                              >
                                <sub.icon className="w-4 h-4" />
                                <span>{sub.label}</span>
                              </Link>
              ...
              <div className="p-4 border-t">
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center gap-3 px-4 py-4 rounded-xl text-red-600 hover:bg-red-50 transition font-semibold"
                >
                  <LogOut className="w-5 h-5" />
                  <span>Logout</span>
                </button>
